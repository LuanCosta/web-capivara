...
async function paged(path:string,fallback:News[]=[]):Promise<NewsPage>{try{const r=await fetch(`${API}${path}`,{cache:"no-store"});if(!r.ok)throw new Error(`API ${r.status}`);const json=await r.json() as Raw;const items=Array.isArray(json.items)?newestFirst((json.items as Raw[]).map(mapNews)):[];return{items,nextCursor:typeof json.nextCursor==="string"?json.nextCursor:null,hasMore:Boolean(json.hasMore)}}catch{return process.env.NODE_ENV==="development"?{items:newestFirst(fallback),nextCursor:null,hasMore:false,unavailable:true}:{items:[],nextCursor:null,hasMore:false,unavailable:true}}}
...
export async function getNewsById(id:string):Promise<News>{try{const r=await fetch(`${API}/feed/${id}`,{cache:"no-store"});if(!r.ok)throw new Error(`API ${r.status}`);return mapNews(await r.json())}catch{const fallback=fallbackNews.find(n=>n.id===id);if(process.env.NODE_ENV==="development"&&fallback)return fallback;throw new Error("Notícia indisponível")}}
