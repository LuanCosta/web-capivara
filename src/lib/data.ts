export const API = "https://bff-capivara.fly.dev/api";
...
export type NewsPage = { items:News[]; nextCursor:string|null; hasMore:boolean; unavailable?:boolean };
...
const safeExternalUrl=(v:unknown)=>{if(typeof v!=="string")return undefined;try{const url=new URL(v);return url.protocol==="https:"||url.protocol==="http:"?url.toString():undefined}catch{return undefined}};
...
async function paged(path:string,fallback:News[]=[],fresh=false):Promise<NewsPage>{try{const r=await fetch(`${API}${path}`,fresh?{cache:"no-store"}:{next:{revalidate:60}});if(!r.ok)throw new Error(`API ${r.status}`);const json=await r.json() as Raw;const items=Array.isArray(json.items)?newestFirst((json.items as Raw[]).map(mapNews)):[];return{items,nextCursor:typeof json.nextCursor==="string"?json.nextCursor:null,hasMore:Boolean(json.hasMore)}}catch{return process.env.NODE_ENV==="development"?{items:newestFirst(fallback),nextCursor:null,hasMore:false,unavailable:true}:{items:[],nextCursor:null,hasMore:false,unavailable:true}}}
export const getNewsPage=(cursor?:string|null,limit=10,fresh=false)=>paged(`/feed/paged?limit=${limit}${cursor?`&cursor=${encodeURIComponent(cursor)}`:""}`,fallbackNews,fresh);
...
export async function getNewsById(id:string):Promise<News>{try{const r=await fetch(`${API}/feed/${id}`,{next:{revalidate:60}});if(!r.ok)throw new Error(`API ${r.status}`);return mapNews(await r.json())}catch{const fallback=fallbackNews.find(n=>n.id===id);if(process.env.NODE_ENV==="development"&&fallback)return fallback;throw new Error("Notícia indisponível")}}
